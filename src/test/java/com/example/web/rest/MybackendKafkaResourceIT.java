...
class MybackendKafkaResourceIT {
    ...
    void setup() {
        ...
        MybackendKafkaResource kafkaResource = new MybackendKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/mybackend-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/mybackend-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
